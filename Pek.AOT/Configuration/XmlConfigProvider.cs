...
public class XmlConfigProvider : FileConfigProvider
{
    ...
    {
        if ((RootName.StrIsNullOrEmpty() || RootName == "Root") && !value.StrIsNullOrEmpty()) RootName = Path.GetFileNameWithoutExtension(value);

        // 加上默认后缀
        if (!value.StrIsNullOrEmpty() && Path.GetExtension(value).StrIsNullOrEmpty()) value += ".config";
    ...
    protected override void OnRead(String fileName, IConfigSection section)
    {
        ...
        if (!reader.Name.StrIsNullOrEmpty()) RootName = reader.Name;
    ...
    private void ReadNode(XmlReader reader, IConfigSection section)
    {
        while (true)
        {
            ...
            // 前一行是注释
            if (!remark.StrIsNullOrEmpty()) cfg.Comment = remark;
    ...
    private void WriteNode(XmlWriter writer, String name, IConfigSection section)
    {
        ...
        {
            if (item.Key.StrIsNullOrEmpty()) continue;

            // 写注释
            if (!item.Comment.StrIsNullOrEmpty()) writer.WriteComment(item.Comment);
            ...
            if (cs != null)
            {
                ...
                if (cs.Count >= 2 && cs[0].Key == cs[1].Key)
                {
                    ...
                    {
                        if (!elm.Key.StrIsNullOrEmpty()) WriteAttributeNode(writer, elm.Key, elm);
                    }
