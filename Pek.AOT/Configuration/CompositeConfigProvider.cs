...
public class CompositeConfigProvider : IConfigProvider
{
    ...
    {
        Name = GetType().Name.Replace("ConfigProvider", "");
    ...
    public virtual void Bind<T>(T model, Boolean autoReload = true, String? path = null)
    {
        ...
        path ??= String.Empty;
        // AOT 兼容：委托给底层提供者的 Bind 方法
        foreach (var cfg in Configs)
        {
            cfg.Bind(model, autoReload, path);
        }

        if (autoReload) _models.TryAdd(model, path);
    ...
    public virtual void Bind<T>(T model, String? path, Action<IConfigSection> onChange)
    {
        ...
        foreach (var cfg in Configs)
        {
            cfg.Bind(model, path, onChange);
        }

        if (onChange != null) _models2.TryAdd(model, new ModelWrap(path, onChange));
    ...
    protected virtual void NotifyChange()
    {
        foreach (var item in _models)
        {
            ...
            foreach (var cfg in Configs)
            {
                cfg.Bind(model, true, item.Value);
            }
        }
