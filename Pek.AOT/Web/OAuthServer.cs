...
public class OAuthServer
{
    #region 属性
    /// <summary>缓存</summary>
    public ICache Cache { get; set; } = Pek.Caching.Cache.Default;
    ...
    /// <returns>随机key，用于后续获取跳转URL</returns>
    ...
    public virtual String GetResult(Int32 key, IManageUser user)
    {
        ...
        if (url.Contains("?"))
            ...
            url += "?";
        url += "code=" + code;
        if (!model.State.StrIsNullOrEmpty()) url += "&state=" + model.State;
    ...
    public String Decode(String token)
    {
        ...
        if (!rs || username.StrIsNullOrEmpty()) throw new Exception("非法访问令牌");
    ...
    private TokenProvider GetProvider()
    {
        ...
        if (prv.Key.StrIsNullOrEmpty()) prv.ReadKey("..\\Keys\\OAuth.prvkey", true);
