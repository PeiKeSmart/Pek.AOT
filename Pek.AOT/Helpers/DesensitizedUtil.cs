...
/// <summary>信息脱敏工具类</summary>
/// <remarks>https://github.com/dotnet-easy/easytool/blob/main/EasyTool.Core/ToolCategory/DesensitizedUtil.cs</remarks>
public static partial class DesensitizedUtil
{
    ...
    /// <summary>脱敏用户ID，只保留前两位和后两位</summary>
    /// <param name="userId">用户ID</param>
    /// <returns>脱敏后的用户ID</returns>
    public static String UserId(String userId)
    {
        if (String.IsNullOrWhiteSpace(userId) || userId.Length <= 4) return userId;
    ...
    /// <summary>脱敏中文姓名，只保留第一个汉字和最后一个汉字，其他用*代替</summary>
    /// <param name="name">中文姓名</param>
    /// <returns>脱敏后的中文姓名</returns>
    public static String ChineseName(String name)
    {
        if (String.IsNullOrWhiteSpace(name) || name.Length <= 2) return name;
    ...
    /// <summary>脱敏身份证号码，只保留前两位和后四位</summary>
    /// <param name="idcard">身份证号码</param>
    /// <returns>脱敏后的身份证号码</returns>
    public static String Idcard(String idcard)
    {
        if (String.IsNullOrWhiteSpace(idcard) || !IdcardRegex.IsMatch(idcard)) return idcard;
    ...
    /// <summary>脱敏座机号码，只保留前三位和后四位</summary>
    /// <param name="tel">座机号码</param>
    /// <returns>脱敏后的座机号码</returns>
    public static String Tel(String tel)
    {
        if (String.IsNullOrWhiteSpace(tel) || !TelRegex.IsMatch(tel)) return tel;
    ...
    /// <summary>脱敏手机号码，只保留前三位和后四位</summary>
    /// <param name="mobile">手机号码</param>
    /// <returns>脱敏后的手机号码</returns>
    public static String Mobile(String mobile)
    {
        if (String.IsNullOrWhiteSpace(mobile) || !MobileRegex.IsMatch(mobile)) return mobile;
    ...
    /// <summary>脱敏地址信息，只保留前五个字符和后三个字符</summary>
    /// <param name="address">地址信息</param>
    /// <returns>脱敏后的地址信息</returns>
    public static String Address(String address)
    {
        if (String.IsNullOrWhiteSpace(address) || address.Length <= 8) return address;
    ...
    /// <summary>脱敏电子邮件，只保留邮箱前缀的前三个字符和后两个字符</summary>
    /// <param name="email">电子邮件</param>
    /// <returns>脱敏后的电子邮件</returns>
    public static String Email(String email)
    {
        if (String.IsNullOrWhiteSpace(email) || !EmailRegex.IsMatch(email)) return email;
    ...
    /// <summary>脱敏银行卡号，只保留前四位和后四位</summary>
    /// <param name="bankcard">银行卡号</param>
    /// <returns>脱敏后的银行卡号</returns>
    public static String Bankcard(String bankcard)
    {
        if (String.IsNullOrWhiteSpace(bankcard) || !BankcardRegex.IsMatch(bankcard)) return bankcard;
    ...
    /// <summary>将传入的字符串中间部分字符替换成特殊字符</summary>
    ...
    public static String ReplaceWithSpecialChar(String value, Int32 startLen = 2, Int32 endLen = 2, Char specialChar = '*')
    {
        try
        {
            ...
            var specialStr = new String(specialChar, replaceStr.Length);
