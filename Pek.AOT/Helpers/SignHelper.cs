...
public class SignHelper
{
    ...
    public static String? GetSign(Dictionary<String, Object> keys, String secret, Boolean isToLower = false)
    {
        ...
        if (isToLower) input = input.ToLower();
        input += secret;
        var sign = input.MD5().ToUpper();
        return sign;
    }

    /// <summary>字符串比较器（按字符ASCII码逐位比较）</summary>
    public class StringCompare : IComparer<KeyValuePair<String, Object>>
    {
        /// <summary>比较两个键值对</summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <returns></returns>
