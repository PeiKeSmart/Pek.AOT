...
public class QueueService<T> : IQueueService<T>
{
    ...
    {
        var dic = _topics.GetOrAdd(topic, static _ => new ConcurrentDictionary<String, IProducerConsumer<T>>());
    ...
    {
        if (_topics.TryGetValue(topic, out var clients) && clients.TryGetValue(clientId, out var queue))
