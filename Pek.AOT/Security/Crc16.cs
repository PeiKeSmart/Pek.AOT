...
public sealed class Crc16
{
    ...
    public Crc16 Reset() { Value = CrcSeed; return this; }
    ...
    public Crc16 Update(Stream stream, Int64 count = -1)
    {
        ...
        while (--count >= 0)
        {
            ...
            crc = (UInt16)((crc << 8) ^ CrcTable[(crc >> 8 ^ value) & 0xFF]);
        }
    ...
    public static UInt16 Compute(Byte[] buffer, Int32 offset = 0, Int32 count = -1)
    {
        var crc = new Crc16();
    ...
    private static readonly UInt16[] _crcTableModbus = [0x0000, 0xCC01, 0xD801, 0x1400, 0xF001, 0x3C00, 0x2800, 0xE401, 0xA001, 0x6C00, 0x7800, 0xB401, 0x5000, 0x9C01, 0x8801, 0x4400];
    ...
    public static UInt16 ComputeModbus(Byte[] data, Int32 offset, Int32 count = -1)
    {
        ...
        {
            var current = data[i];
            value = (UInt16)(_crcTableModbus[(current ^ value) & 15] ^ (value >> 4));
            value = (UInt16)(_crcTableModbus[((current >> 4) ^ value) & 15] ^ (value >> 4));
        }
