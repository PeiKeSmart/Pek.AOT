...
public interface IPasswordProvider
{
    ...
    /// <param name="password">传输密码，可能是明文或 MD5</param>
...
public class SaltPasswordProvider : IPasswordProvider
{
    ...
    public virtual Boolean Verify(String password, String hash)
    {
        ...
        switch (ss[1])
        {
            ...
            case "md5+sha1":
                ...
                if (ss[3] == DecodeHex(password).SHA1(salt.GetBytes()).ToBase64()) return true;
                return false;
            case "md5+sha512":
                ...
                if (ss[3] == DecodeHex(password).SHA512(salt.GetBytes()).ToBase64()) return true;
                return false;
            default:
    ...
    private static Byte[] DecodeHex(String value)
    {
        ...
        if ((charCount & 1) != 0) return value.GetBytes();

        var byteCount = charCount / 2;
        Byte[]? rented = null;
        Span<Byte> buffer = byteCount <= 256 ? stackalloc Byte[byteCount] : (rented = ArrayPool<Byte>.Shared.Rent(byteCount));
        try
        {
            var status = Convert.FromHexString(value, buffer, out var charsConsumed, out var bytesWritten);
            if (status != OperationStatus.Done || charsConsumed != charCount)
