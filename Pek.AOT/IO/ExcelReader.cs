...
public class ExcelReader : DisposeBase
{
    ...
    private IDictionary<String, ZipArchiveEntry> ReadSheets(ZipArchive zip)
    {
        ...
        foreach (var item in zip.Entries)
        {
            ...
            {
                var name = item.Name;
                if (name.EndsWith(".xml", StringComparison.OrdinalIgnoreCase)) name = name[..^4];
                if (dic.TryGetValue(name.StartsWith("sheet", StringComparison.OrdinalIgnoreCase) ? name[5..] : name, out var str)) name = str;
                name ??= String.Empty;
