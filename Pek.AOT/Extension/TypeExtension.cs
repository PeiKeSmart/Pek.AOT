...
/// <remarks>
/// AOT 兼容说明：
/// - type.Unwrap() 替换为 Nullable.GetUnderlyingType(type) ?? type
/// - GetConstructor(params Type[]) 依赖 Pek.Helpers.ActivatorHelper，已跳过
/// - 其余方法均为元数据级别检查，AOT 安全
/// </remarks>
public static class TypeExtension
{
    ...
    /// <summary>
    /// 判断是否基础类型（AOT 兼容：type.Unwrap() 替换为 Nullable.GetUnderlyingType）
    /// </summary>
    /// <param name="type">type</param>
    /// <returns></returns>
    ...
    // GetConstructor(params Type[]) 未迁移：
    // 上游 Pek.Common 版本依赖 Pek.Helpers.ActivatorHelper.FindApplicableConstructor，
    // 该辅助类在 Pek.AOT 中不可用。如需按参数类型查找构造函数，请使用 GetMatchingConstructor。
