...
public static class ProcessHelper
{
    ...
    private static String? GetCommandLineOnWindows(Int32 processId)
    {
        ...
        try
        {
            ...
            PEB peb;
            if (!ReadStruct(processHandle, pbi.PebBaseAddress, out peb)) return null;

            RtlUserProcessParameters upp;
            if (!ReadStruct(processHandle, peb.ProcessParameters, out upp)) return null;
    ...
    private static Boolean ReadStruct(IntPtr hProcess, IntPtr lpBaseAddress, out PEB val)
    {
        val = default!;
        var size = Marshal.SizeOf<PEB>();
        var ptr = Marshal.AllocHGlobal(size);
        try
        {
            if (ReadProcessMemory(hProcess, lpBaseAddress, ptr, (UInt32)size, out var len) && len == size)
            {
                val = Marshal.PtrToStructure<PEB>(ptr);
                return true;
            }
        }
        finally
    ...
    private static Boolean ReadStruct(IntPtr hProcess, IntPtr lpBaseAddress, out RtlUserProcessParameters val)
    {
        val = default!;
        var size = Marshal.SizeOf<RtlUserProcessParameters>();
        var ptr = Marshal.AllocHGlobal(size);
        try
        {
            if (ReadProcessMemory(hProcess, lpBaseAddress, ptr, (UInt32)size, out var len) && len == size)
            {
                val = Marshal.PtrToStructure<RtlUserProcessParameters>(ptr);
                return true;
            }
        }
        finally
